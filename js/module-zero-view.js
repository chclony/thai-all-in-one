/* Thai All-in-One — ekran Modułu 0.
   ...
   patrzenia na ekran. Stąd trzy decyzje, które przenikają cały ten plik:
     ...
     1. Wszystkie przyciski są duże i osiągalne kciukiem — moduł jest
        pomyślany na telefon. Obsługa klawiaturą działa nadal (cyfry, R, P,
        Enter) i jest potrzebna dla dostępności, ale NIE jest pokazywana
        w interfejsie: podpowiedź o klawiszach na telefonie to szum.
...
(function (global) {
  ...
  var View = {};
  var state = {
    mode: 'map',       // 'map' | 'brief' | 'lesson' | 'diagnostic'
    brief: null,       // lekcja, której odprawę właśnie czytamy
    keysBound: false,
  ...
  View.render = function () {
    ...
    if (state.mode === 'brief' && state.brief) { renderBrief(root, state.brief); return; }
  ...
  /* Odprawa lekcji — własna, albo pożyczona z lekcji wskazanej w briefRef.
     Lekcje 11 i 12 wracają do tonów w dłuższym materiale i korzystają z tej
     samej odprawy co lekcja 1, zamiast powielać jej treść w danych. */
  function briefOf(L) {
    if (!L) return null;
    if (L.brief) return L.brief;
    if (L.briefRef) {
      var src = Perception.byId(L.briefRef);
      return (src && src.brief) || null;
    }
    return null;
  }

  /* Do sesji XIV lekcja 1 od razu pytała „jaki to ton?” — o dźwięku, którego
     uczący się nigdy świadomie nie słyszał. To nie jest ćwiczenie, tylko
     zgadywanie z pięcioma opcjami. Jeżeli lekcja ma odprawę, pokazujemy ją
     przed pierwszym zadaniem; przy powtórce lekcji da się ją pominąć jednym
     przyciskiem, bo za drugim razem już się jej nie czyta. */
  function startLesson(L, skipBrief) {
    var brief = briefOf(L);
    if (brief && !skipBrief) {
      state.brief = { lesson: L, data: brief };
      state.mode = 'brief';
      View.render();
      return;
    }
  ...
  /* Rysunek konturu tonu na skali Chao (1 = dół, 5 = góra). Sam opis słowny
     nie wystarcza: „opada, potem się wznosi” brzmi tak samo jak „opada”,
     dopóki nie zobaczy się kształtu. Wykres jest maleńki i czysto poglądowy,
     więc dla czytnika ekranu opisujemy go słowami w aria-label. */
  function toneGlyph(tone) {
    var W = 76, H = 40, pad = 5;
    var pts = (tone.contour || []).map(function (lvl, i, arr) {
      var x = pad + (arr.length === 1 ? (W - 2 * pad) / 2
        : i * (W - 2 * pad) / (arr.length - 1));
      var y = pad + (5 - lvl) * (H - 2 * pad) / 4;
    ...
    svg.setAttribute('class', 'm0-tone-glyph');
    svg.setAttribute('role', 'img');
    svg.setAttribute('aria-label', tone.name + ': ' + tone.shape);

    [1, 3, 5].forEach(function (lvl) {
      var y = pad + (5 - lvl) * (H - 2 * pad) / 4;
      var ln = document.createElementNS(ns, 'line');
      ln.setAttribute('x1', pad); ln.setAttribute('x2', W - pad);
      ln.setAttribute('y1', y); ln.setAttribute('y2', y);
      ln.setAttribute('class', 'm0-glyph-grid');
      svg.appendChild(ln);
    });
    var line = document.createElementNS(ns, 'polyline');
    line.setAttribute('points', pts);
    line.setAttribute('class', 'm0-glyph-line');
    svg.appendChild(line);
    return svg;
  }

  /* Przycisk „posłuchaj” dla pojedynczego bodźca albo serii. */
  function listenBtn(ids, label, cls) {
    var b = U.el('button', { class: 'btn ' + (cls || 'ghost'), type: 'button',
      'aria-label': label });
    b.appendChild(U.icon('play'));
    b.appendChild(U.el('span', { text: label }));
    b.addEventListener('click', function () { playSeries(ids, { btn: b }); });
    return b;
  }

  function renderBrief(root, ctx) {
    var L = ctx.lesson, brief = ctx.data;

    var head = U.el('div', { class: 'card' });
    head.appendChild(U.el('h2', { text: brief.title }));
    head.appendChild(U.el('p', { text: brief.intro }));
    head.appendChild(U.el('p', { class: 'm0-keyidea', text: brief.keyIdea }));
    root.appendChild(head);

    /* --- pięć tonów, każdy z etykietą OD RAZU widoczną ------------------
       Cała różnica względem zadania: tutaj wiesz, czego słuchasz. Ucho uczy
       się przez skojarzenie dźwięku z nazwą, a nie przez trafianie w ciemno. */
    var list = U.el('div', { class: 'card' });
    list.appendChild(U.el('h3', { text: 'Pięć tonów po kolei' }));
    (brief.tones || []).forEach(function (t) {
      var row = U.el('div', { class: 'm0-tone-row' });

      var fig = U.el('div', { class: 'm0-tone-fig' });
      fig.appendChild(toneGlyph(t));
      fig.appendChild(U.el('span', { class: 'm0-chao', text: t.chao }));
      row.appendChild(fig);

      var body = U.el('div', { class: 'm0-tone-body' });
      body.appendChild(U.el('p', { class: 'm0-tone-name', text: t.name }));
      body.appendChild(U.el('p', { class: 'muted', text: t.shape }));
      body.appendChild(U.el('p', { text: t.analogy }));
      body.appendChild(U.el('p', { class: 'm0-listen-for', text: t.listenFor }));

      var st = stimOf(t.stimulusId);
      if (st) {
        body.appendChild(U.el('p', { class: 'bc-th', text: st.phonetic
          + ' — ' + st.polish }));
        body.appendChild(U.el('div', { class: 'btn-row' },
          [listenBtn([t.stimulusId], 'Posłuchaj: ' + t.name)]));
      }
      row.appendChild(body);
      list.appendChild(row);
    });
    var allIds = (brief.tones || []).map(function (t) { return t.stimulusId; });
    list.appendChild(U.el('div', { class: 'btn-row' },
      [listenBtn(allIds, 'Odtwórz wszystkie pięć po kolei', 'gold')]));
    root.appendChild(list);

    /* --- zestaw minimalny: ta sama sylaba, inny ton, inne słowo --------- */
    if (brief.minimalSet) {
      var ms = U.el('div', { class: 'card' });
      ms.appendChild(U.el('h3', { text: brief.minimalSet.title }));
      ms.appendChild(U.el('p', { text: brief.minimalSet.note }));
      var ids = [];
      (brief.minimalSet.items || []).forEach(function (it) {
        var st = stimOf(it.stimulusId);
        if (!st) return;
        ids.push(it.stimulusId);
        var row = U.el('div', { class: 'm0-min-row' });
        row.appendChild(U.el('span', { class: 'bc-th', text: st.phonetic }));
        row.appendChild(U.el('span', { class: 'muted', text: 'ton ' + it.tone }));
        row.appendChild(U.el('span', { text: st.polish }));
        row.appendChild(listenBtn([it.stimulusId], 'Posłuchaj: ' + st.phonetic));
        ms.appendChild(row);
      });
      if (ids.length) {
        ms.appendChild(U.el('div', { class: 'btn-row' },
          [listenBtn(ids, 'Odtwórz cały zestaw', 'gold')]));
      }
      root.appendChild(ms);
    }

    /* --- pary mylące: nauka przez kontrast, nie przez listę piątki ------ */
    if (brief.pairs) {
      var pr = U.el('div', { class: 'card' });
      pr.appendChild(U.el('h3', { text: brief.pairs.title }));
      pr.appendChild(U.el('p', { text: brief.pairs.note }));
      (brief.pairs.items || []).forEach(function (it) {
        var row = U.el('div', { class: 'm0-pair-row' });
        row.appendChild(U.el('p', { class: 'm0-tone-name',
          text: it.a + '  kontra  ' + it.b }));
        row.appendChild(U.el('p', { text: it.why }));
        row.appendChild(U.el('div', { class: 'btn-row' }, [
          listenBtn([it.aId, it.bId], 'Posłuchaj obu po kolei'),
          listenBtn([it.aId], it.a),
          listenBtn([it.bId], it.b)
        ]));
        pr.appendChild(row);
      });
      root.appendChild(pr);
    ...
    if ((brief.tips || []).length) {
      var tp = U.el('div', { class: 'card' });
      tp.appendChild(U.el('h3', { text: 'Zanim zaczniesz' }));
      var ul = U.el('ul', { class: 'm0-tips' });
      brief.tips.forEach(function (t) { ul.appendChild(U.el('li', { text: t })); });
      tp.appendChild(ul);
      root.appendChild(tp);
    }
    ...
    var go = U.el('button', { class: 'btn gold', type: 'button',
      text: resuming ? 'Wracam do zadania' : 'Rozumiem — zaczynam ćwiczenie' });
    go.addEventListener('click', function () {
      Player.stop();
      state.brief = null;
      /* Przy powrocie w trakcie lekcji przebieg trwa nadal — wystarczy wrócić
         do trybu 'lesson'. Ponowne Perception.start skasowałoby odpowiedzi. */
      if (resuming) { state.mode = 'lesson'; View.render(); return; }
      startLesson(L, true);
    });
    var back = U.el('button', { class: 'btn ghost', type: 'button',
      text: resuming ? 'Przerwij lekcję' : 'Wróć do mapy' });
    back.addEventListener('click', function () {
      Player.stop();
      state.brief = null;
      if (resuming) Perception.run = null;
      state.mode = 'map';
      View.render();
    });
    root.appendChild(U.el('div', { class: 'btn-row' }, [go, back]));
  }
  ...
  function renderLessonRun(root) {
    ...
    /* Moment, w którym człowiek chce wrócić do wyjaśnienia, to nie początek
       lekcji, tylko chwila, w której właśnie nie potrafi nazwać tonu. Dlatego
       odprawa jest osiągalna z wnętrza zadania — bez przerywania lekcji
       i bez utraty postępu, bo wracamy dokładnie do tego samego zadania. */
    var brief = briefOf(run.lesson);
    if (brief && task.type === 'tone-scale') {
      var remind = U.el('button', { class: 'btn ghost', type: 'button',
        text: 'Nie wiem, jak brzmi ten ton — przypomnij' });
      remind.addEventListener('click', function () {
        Player.stop();
        state.brief = { lesson: run.lesson, data: brief, resume: true };
        state.mode = 'brief';
        View.render();
      });
      root.appendChild(U.el('div', { class: 'btn-row' }, [remind]));
    }
  ...
  function renderTask(box, task, onAnswer, onNext, pass) {
    ...
    /* Sterowanie dźwiękiem. Przycisk odtworzenia dostaje focus od razu —
       na telefonie jest pierwszy pod kciukiem, a czytnik ekranu czyta go
       od razu po wejściu w zadanie. */
    var playBtn = U.el('button', { class: 'btn gold play-btn', type: 'button',
      'aria-pressed': 'false',
      'aria-label': 'Odtwórz ponownie' });
    ...
    task.options.forEach(function (label, i) {
      ...
      /* Numerek przy odpowiedzi był podpowiedzią skrótu klawiszowego. Na
         telefonie nie znaczy nic i tylko konkuruje z treścią przycisku,
         więc znika z widoku — obsługa cyfrą działa dalej dla klawiatury. */
      b.appendChild(U.el('span', { text: label }));
    ...
    function showResult(ok, chosen) {
      ...
      if (task.compare && task.compare.length >= 2) {
        var cmp = U.el('button', { class: 'btn', type: 'button',
          'aria-label': 'Porównaj oba warianty jeden po drugim' });
      ...
      var again = U.el('button', { class: 'btn ghost', type: 'button',
        'aria-label': 'Odtwórz zadanie jeszcze raz', text: 'Jeszcze raz' });
