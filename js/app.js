...
(function (global) {
  ...
  /* --------------------------------------------------- wiersz i szczegóły */
  /* Wiersz wyniku — dwie NIEZALEŻNE akcje, a nie jedna udająca dwie.

     Do sesji XV cały wiersz był jednym <button>, a ikona głośnika w nim
     zwykłym <span aria-hidden>. Wyglądała jak przycisk odtwarzania i nie
     robiła nic: kliknięcie w nią bąbelkowało do wiersza i otwierało hasło.
     Żeby usłyszeć słowo, trzeba było wejść w szczegóły i dopiero tam kliknąć
     „Posłuchaj” — czyli najczęstsza czynność w słowniku wymagała dwóch
     kroków, a element, który ją obiecywał, był atrapą.

     Teraz wiersz jest kontenerem <div> z dwoma prawdziwymi przyciskami:
     treść otwiera hasło, ikona odtwarza dźwięk. Zagnieżdżanie <button>
     w <button> jest niedozwolone, więc kontener musiał przestać być
     przyciskiem — stąd ta zmiana, a nie samo dopięcie obsługi zdarzenia. */
  function recordRow(source) {
    var rec = G.view(source);
    var row = U.el('div', { class: 'row' });

    var main = U.el('button', { class: 'row-main', type: 'button',
      'aria-label': 'Otwórz hasło: ' + rec.polish }, [
    ...
    main.addEventListener('click', function () {
      App.openRecord(source.id, { btn: main });
    });
    row.appendChild(main);

    /* Nazwa dostępna mówi CO zabrzmi, nie tylko że coś zabrzmi — w liście
       kilkudziesięciu wierszy samo „Posłuchaj” nie odróżnia jednego od
       drugiego. Odtwarzamy `source`, nie `rec`: Player sam wybiera wariant
       płci i nagranie, a G.view podmienia tylko to, co idzie na ekran. */
    var play = U.el('button', { class: 'icon-btn row-play', type: 'button',
      'aria-pressed': 'false',
      'aria-label': 'Posłuchaj: ' + rec.polish });
    play.appendChild(U.icon('play'));
    play.addEventListener('click', function (e) {
      e.stopPropagation();
      Player.play(source, { btn: play });
    });
  ...
  App.openRecord = function (id, opts) {
    opts = opts || {};
    var source = DB.get(id);
    /* Wyniki słownika to skróty z indeksu — pełny rekord leży w pliku poziomu,
       który mógł jeszcze nie zostać dociągnięty. Do sesji XV kończyło się to
       cichym `return`: kliknięcie w wynik nie robiło DOSŁOWNIE nic i nie było
       jak zgadnąć dlaczego. Dociągamy plik i otwieramy hasło, gdy dojedzie —
       tak samo, jak od dawna robi to odtwarzanie dźwięku. */
    if (!source && DB.ensureFor) {
      /* Pobranie pliku poziomu potrafi trwać sekundę albo trzy. Bez sygnału
         wygląda to identycznie jak zepsuty przycisk, więc na czas czekania
         wciskamy klikniętą pozycję — tak samo jak przy odtwarzaniu dźwięku. */
      var waiting = opts.btn || null;
      if (waiting) waiting.classList.add('loading');
      DB.ensureFor([id]).then(function () {
        if (waiting) waiting.classList.remove('loading');
        if (DB.get(id)) App.openRecord(id, opts);
        else U.toast('Nie udało się wczytać tego hasła.');
      });
      return;
    }
    if (!source) return;
